...
- (void)viewDidAppear:(BOOL)animated{
    ...
    [DKHelper showAlertWithTitle:@"重要提示" message:@"本软件绿色安全，插件功能仅供装逼，由本软件产生的任何利益纠纷须有使用者自行承担。在收到微信团队\"非法客户端提示后\"继续使用可能有封号风险，需使用者自行承担。如遇到提醒，请卸载本软件，更换官方微信客户端" btnTitle:@"我明白了" handler:^(UIButton *sender) {
...
- (void)reloadTableData{
    ...
    redEnvelopSection.headerTitle = @"自动抢红包设置(延迟1.5秒)";
    ...
    supportAuthorSection.headerTitle = @"赞赏可以开启安全模式(防封号)";
    [manager addSection:supportAuthorSection];
    WCTableViewNormalCellManager * payMeCell = [DKHelper cellWithSel:@selector(payForMe) target:self title:@"开启高级功能👍👍👍"];
