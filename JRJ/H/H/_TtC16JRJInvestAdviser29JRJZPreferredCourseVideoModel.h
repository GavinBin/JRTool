//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "_TtC16JRJInvestAdviser12JRJBaseModel.h"

@class NSString;

@interface _TtC16JRJInvestAdviser29JRJZPreferredCourseVideoModel : _TtC16JRJInvestAdviser12JRJBaseModel
{
    // Error parsing type: , name: url
    // Error parsing type: , name: title
    // Error parsing type: , name: profile
    // Error parsing type: , name: duration
    // Error parsing type: , name: src
}

- (void).cxx_destruct;
- (id)init;
@property(nonatomic, copy) NSString *src;
@property(nonatomic, copy) NSString *duration;
@property(nonatomic, copy) NSString *profile;
@property(nonatomic, copy) NSString *title;
@property(nonatomic, retain) NSString *url; // @synthesize url;

@end

