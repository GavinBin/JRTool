//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "_TtC16JRJInvestAdviser10ZKBaseCell.h"

@class UILabel;

@interface _TtC16JRJInvestAdviser33JRJDecisionStockPoolFiveLabelCell : _TtC16JRJInvestAdviser10ZKBaseCell
{
    // Error parsing type: , name: oneLabel
    // Error parsing type: , name: twoLabel
    // Error parsing type: , name: threeLabel
    // Error parsing type: , name: fourLabel
    // Error parsing type: , name: fiveLabel
}

- (void).cxx_destruct;
- (id)initWithCoder:(id)arg1;
- (id)initWithStyle:(long long)arg1 reuseIdentifier:(id)arg2;
- (void)setLabelColorWithLabel:(id)arg1 type:(long long)arg2;
- (void)updateViewForModelWithModel:(id)arg1;
- (void)awakeFromNib;
@property(nonatomic) __weak UILabel *fiveLabel; // @synthesize fiveLabel;
@property(nonatomic) __weak UILabel *fourLabel; // @synthesize fourLabel;
@property(nonatomic) __weak UILabel *threeLabel; // @synthesize threeLabel;
@property(nonatomic) __weak UILabel *twoLabel; // @synthesize twoLabel;
@property(nonatomic) __weak UILabel *oneLabel; // @synthesize oneLabel;

@end

