//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "JRJListTableViewController.h"

@interface ViewPointBaseController : JRJListTableViewController
{
    _Bool _isHadSigned;
}

@property(nonatomic) _Bool isHadSigned; // @synthesize isHadSigned=_isHadSigned;
- (void)tableView:(id)arg1 didSelectRowAtIndexPath:(id)arg2;
- (id)tableView:(id)arg1 cellForRowAtIndexPath:(id)arg2;
- (double)tableView:(id)arg1 heightForRowAtIndexPath:(id)arg2;
- (double)tableView:(id)arg1 heightForHeaderInSection:(long long)arg2;
- (void)requestMoreListData;
- (void)refreshViewBeginRefresh:(id)arg1;
- (void)handleViewPointDataArray:(id)arg1 withDirectionUp:(_Bool)arg2;
- (void)requestViewPointListWithDirectionUp:(_Bool)arg1;
- (void)showAttentionGuideView;
- (void)loadListRequest;
- (id)getRequestURLWithDirectionUp:(_Bool)arg1;
- (void)viewWillAppear:(_Bool)arg1;

@end

